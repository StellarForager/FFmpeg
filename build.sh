...
[ ! -f config.status ] && \
  ...
  --extra-ldflags="-L$TARGET_DIR/lib -Wl,-s $([ "$PLATFORM" = "darwin" ] && echo "-dead_strip") $([ "$PLATFORM" != "darwin" ] && echo "--Wl,--gc-sections -Wl,--as-needed -Wl,--build-id=none") $([ "$PLATFORM" = "mingw32" ] && echo "-static")" \
